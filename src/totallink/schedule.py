...
if TYPE_CHECKING:
    ...
    from totallink.weight import WeightItem
...
from totallink.util import datetime2epoch, epoch2datetime, schedulerLink, epoch2ISO8601, ISO86012epoch, plusLink
from totallink.weight import Display, GreenDisplay, RoastedDisplay, PROCESS_STATE, WeightManager, GreenWeightItem, RoastedWeightItem
...
#        QTimer.singleShot(50, self.setActiveTab)
    ...
    def schedule_changed_significantly(self, old_schedule:list[ScheduledItem], new_schedule:list[totallink.stock.ScheduledItem]) -> bool:
...
#            _log.exception(e) # this can raise an exception on macOS if Artisan is started using sudo as the logging framework might not fully initialized
    ...
    @staticmethod
    def same_blend(blend_list1:totallink.stock.BlendList|None, blend2:totallink.stock.Blend|None) -> bool:
        if blend_list1 is not None and blend2 is not None:
            blend1 = totallink.stock.list2blend(blend_list1)
            if blend1 is not None:
                return totallink.stock.matchBlendDict(blend1, blend2, sameLabel=False)
        return False
            ...
            try:
                ...
                if self.aw.plus_account is None:
                    ...
                    if self.scheduled_items == []:
                        ...
                        self.remaining_message.setText(QApplication.translate('Plus', 'Schedule empty!{}Plan your schedule on {}').format('<BR><BR>', f'<a href="{schedulerLink()}">{totallink.config.app_name}</a><br>'))
