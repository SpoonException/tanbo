...
from PyQt6.QtCore import Qt, pyqtSignal, pyqtSlot, QRegularExpression, QSettings, QTimer, QEvent, QLocale, \
    QSignalBlocker
...
class volumeCalculatorDlg(ArtisanDialog):
    def __init__(self, parent: 'editGraphDlg', aw: 'ApplicationWindow', weightIn: float | None, weightOut: float | None,
                 ...
                 inlineedit: QLineEdit, outlineedit: QLineEdit,
                 tare: float) -> None:  # weight in and out expected in g (int)
            ...
            if self.weightIn is None or line.strip() == '':
                ...
                if inWeight == 0:
                    ...
                    self.inVolume = convertVolume(
                        convertWeight(self.weightIn, self.weightunit, 0) * float(
                            comma2dot(self.unitvolumeEdit.text())) / inWeight,
                        5,
                        self.volumeunit)
            ...
            if self.weightOut is None or line.strip() == '':
                ...
                if outWeight == 0:
                    ...
                    self.outVolume = convertVolume(
                        convertWeight(self.weightOut, self.weightunit, 0) * float(
                            comma2dot(str(self.unitvolumeEdit.text()))) / outWeight,
                        5,
                        self.volumeunit)
...
class editGraphDlg(ArtisanResizeablDialog):
    ...
    def __init__(self, parent: QWidget, aw: 'ApplicationWindow', activeTab: int = 0,
        ...
        if self.aw.qmc.timeindex[6] and self.aw.qmc.timeindex[6] < len(self.aw.qmc.timex):
            t7 = self.aw.qmc.timex[self.aw.qmc.timeindex[6]] - (
                0 if self.aw.qmc.timeindex[0] == -1 else self.aw.qmc.timex[self.aw.qmc.timeindex[0]])
        else:
            t7 = 0
        ...
        self.roastpropertiesAutoOpen.setToolTip(
            QApplication.translate('Tooltip', 'Open roast properties dialog on CHARGE'))
        ...
        self.roastpropertiesAutoOpenDROP.setToolTip(
            QApplication.translate('Tooltip', 'Open roast properties dialog on DROP'))
        ...
        self.copyeventTableButton.setToolTip(
            QApplication.translate('Tooltip', 'Copy table to clipboard, OPTION or ALT click for tabular text'))
        ...
        # Scan —— 图标内嵌在输入框右侧
        self.batchScanEdit = QLineEdit()
        self.batchScanEdit.setPlaceholderText(
            QApplication.translate('Label', 'Scan ApsNum')
        )
        self.batchScanEdit.setMinimumHeight(40)
        self.batchScanEdit.setMinimumWidth(180)
        self.batchScanEdit.setMaximumWidth(200)

        basedir = os.path.join(getResourcePath(), 'Icons')
        scanAction = self.batchScanEdit.addAction(
            QIcon(os.path.join(basedir, 'qrcode.svg')),
        ...
        # 回车也触发同一个逻辑
        self.batchScanEdit.returnPressed.connect(scanAction.trigger)

        # self.batchScanEdit.returnPressed.connect(self.onBatchScanned)
        ...
        aps_root.setContentsMargins(0, 4, 0, 4)
        aps_root.setSpacing(30)

        # ── 表头 ──
        aps_header_group = QGroupBox('APS 批次信息')
        aps_header_grid = QGridLayout(aps_header_group)
        ...
        self.weightinedit.setValidator(self.aw.createCLocaleDoubleValidator(0., 9999999., 4,
                                                                            self.weightinedit))  # the max limit has to be high enough otherwise the connected signals are not send!
        ...
        self.weightoutedit.setValidator(self.aw.createCLocaleDoubleValidator(0., 9999999., 4,
                                                                             self.weightoutedit))  # the max limit has to be high enough otherwise the connected signals are not send!
        ...
        # defects
        dw = (self.aw.qmc.roasted_defects_weight if (
                self.aw.qmc.roasted_defects_mode or self.aw.qmc.roasted_defects_weight == 0) else
              (0 if self.aw.qmc.weight[1] == 0 else min(self.aw.qmc.weight[1], max(0.0, self.aw.qmc.weight[
                  1] - self.aw.qmc.roasted_defects_weight))))
        defectsw = f'{float2floatWeightVolume(dw):g}'
        self.weightoutdefectsedit = QLineEdit()
        self.weightoutdefectsedit.setToolTip(QApplication.translate('Tooltip',
                                                                    'weight of defects sorted from roasted coffee or weight of roasted coffee after defects have been removed'))
        ...
        self.weightoutdefectsedit.setValidator(self.aw.createCLocaleDoubleValidator(0., 9999999., 4,
                                                                                    self.weightoutdefectsedit))  # the max limit has to be high enough otherwise the connected signals are not send!
        ...
        self.volumeinedit.setValidator(self.aw.createCLocaleDoubleValidator(0., 999999., 4,
                                                                            self.volumeinedit))  # the max limit has to be high enough otherwise the connected signals are not send!
        ...
        self.volumeoutedit.setValidator(self.aw.createCLocaleDoubleValidator(0., 999999., 4,
                                                                             self.volumeoutedit))  # the max limit has to be high enough otherwise the connected signals are not send!
        ...
        self.bean_density_in_edit.setValidator(
            self.aw.createCLocaleDoubleValidator(0., 999999., 1, self.bean_density_in_edit))
        ...
        self.bean_density_out_edit.setValidator(
            self.aw.createCLocaleDoubleValidator(0., 999999., 1, self.bean_density_out_edit))
        ...
        self.organicpercentlabel.setToolTip(
            QApplication.translate('Tooltip', 'loss of organic matters caused by roasting'))
        ...
        volumeCalcButton.setToolTip(QApplication.translate('Tooltip',
                                                           'Volume calculator to determine coffee volume from sample weight measured in container of known volume'))
        ...
        self.addRecentButton.setToolTip(
            QApplication.translate('Tooltip', 'Add roast properties to list of recent roasts'))
        ...
        self.delRecentButton.setToolTip(
            QApplication.translate('Tooltip', 'Remove roast properties from list of recent roasts'))
        ...
        self.ground_color_edit.setToolTip(
            QApplication.translate('Tooltip', 'color measurement of ground roasted beans'))
        ...
        self.greens_temp_edit.setValidator(self.aw.createCLocaleDoubleValidator(-9999., 999999., 1,
                                                                                self.greens_temp_edit))  # range to 1000 needed to trigger editing_finished on input "12,2"
        ...
        self.moisture_greens_edit.setValidator(
            self.aw.createCLocaleDoubleValidator(0., 100., 1, self.moisture_greens_edit))
        ...
        self.moisture_roasted_edit.setValidator(
            self.aw.createCLocaleDoubleValidator(0., 100., 1, self.moisture_roasted_edit))
        ...
        self.ambient_humidity_edit.setValidator(
            self.aw.createCLocaleDoubleValidator(0., 9999999., 1, self.ambient_humidity_edit))
        ...
        self.ambientedit.setValidator(self.aw.createCLocaleDoubleValidator(-9999., 9999999., 1,
                                                                           self.ambientedit))  # larger range needed to trigger editing_finished
        ...
        self.scaleWeightAccumulated.setToolTip(
            QApplication.translate('Tooltip', 'accumulated weight received from connected scale'))
        ...
        self.defectsButton.setToolTip(
            QApplication.translate('Tooltip', 'set scale weight as weight of defects or yield'))
        self.defectsButton.setText(
            QApplication.translate('Button', 'defects') if self.aw.qmc.roasted_defects_mode else QApplication.translate(
                'Button', 'yield'))
        ...
        updateAmbientTemp.setToolTip(QApplication.translate('Tooltip',
                                                            'retreive ambient data from connected devices or calculate from selected profile curve'))
        ...
        datebatch = QHBoxLayout()
        datebatch.addWidget(dateedit)
        datebatch.addSpacing(20)
        datebatch.addStretch()
        # 新增扫描框
        datebatch.addWidget(self.batchScanEdit)
        datebatch.addSpacing(15)
        datebatch.addStretch()
        ...
        self.plus_blend_selected_spec_labels: list[
                                                  str] | None = None  # the list of coffee labels of the selected blend specification
        if self.aw.plus_account is not None:
            totallink.stock.init()  # we try to init the stock from the cache before populating the popups
            # variables populated by stock data as rendered in the corresponding popups
            self.plus_stores: list[tuple[str, str]] | None = None
            self.plus_coffees: list[tuple[str, tuple[totallink.stock.Coffee, totallink.stock.StockItem]]] | None = None
            self.plus_blends: list[totallink.stock.BlendStructure] | None = None
            ...
            if self.aw.qmc.plus_coffee is not None:
                ...
                if 'ingredients' in self.aw.qmc.plus_blend_spec:  # pyrefly: ignore
                    ...
                    for i in self.aw.qmc.plus_blend_spec['ingredients']:  # pyrefly: ignore
                        c = totallink.stock.getCoffee(i['coffee'])
                        if c is None:
            ...
            self.plus_selected_line.setVisible(False)
            ...
            self.plus_coffees_combo.setSizeAdjustPolicy(
            ...
            self.plusLineStoresFrame.setVisible(False)  # only visible if a blend is selected
            ...
            self.label_origin_flag.setVisible(False)

            self.label_origin_flag.setToolTip(QApplication.translate('Tooltip',
                                                                     "Beans are listed as 'origin, name' if ticked, otherwise as 'name, origin'"))
            self.label_origin_flag.setChecked(bool(totallink.stock.coffee_label_normal_order))
        ...
        self.beansedit.setFixedHeight(30)
        ...
        propGrid.addWidget(self.weightoutdefects_unit_label, 2, 3,
                           Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)
        propGrid.addWidget(self.weightoutdefectspercentlabel, 2, 4,
        ...
        if self.aw.ui_mode is not UI_MODE.PRODUCTION:
            ...
            propGrid.addWidget(self.volumepercentlabel, 3, 4,
                               Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        ...
        propGrid.addWidget(self.scaleWeightAccumulated, 3, 8, 1, 2,
                           Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)
        ...
        propGrid.addWidget(self.bean_density_out_edit, 4, 2,
                           Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        ...
        propGrid.addWidget(self.moisture_roasted_edit, 5, 2,
                           Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
        propGrid.addWidget(moisture_greens_unit_label, 5, 3,
                           Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)
        ...
        if start_recording_on_exit:
            ...
            string = QApplication.translate('Message', 'TotalLINK needs to know the beans you are roasting')
            mbox = QMessageBox(self.aw)
            mbox.setText(string)
            totallink.util.setPlusIcon(mbox)
        ...
        self.stockWorker: totallink.stock.Worker | None = None
        try:
            if self.aw.plus_account is not None:
                self.stockWorker = totallink.stock.getWorker()
                if self.stockWorker is not None:
                    self.updateStockSignalConnection = self.stockWorker.updatedSignal.connect(
                        self.populatePlusCoffeeBlendCombos)
                    QTimer.singleShot(10, totallink.stock.update)
    ...
    def getWeight(self):
        ...
        try:
            if client.connect():
                # float 需要读取两个寄存器
                result = client.read_holding_registers(address=config.registerAddressCharge, count=2)
    ...
    def updateWeightOutDefectsLabel(self) -> None:
        self.defectslabel.setText(
            f"<b>{QApplication.translate('Label', 'Defects')}</b>" if self.aw.qmc.roasted_defects_mode else
    ...
    def customBlendButton_triggered(self, _: bool = False) -> None:
        ...
        coffees = totallink.stock.getCoffeeLabels()
        ...
        if len(coffees) > 2:
            if self.aw.qmc.plus_custom_blend is not None and self.aw.qmc.plus_custom_blend.isValid(coffees.values()):
                ...
                blend = totallink.blend.CustomBlend(QApplication.translate('Form Caption', 'Custom Blend'), [
                    totallink.blend.Component(coffee_tuples[0][1], 0.5),
                    totallink.blend.Component(coffee_tuples[1][1], 0.5)
                ])
            ...
            if self.plus_coffees is not None:
                coffee_hr_ids_with_stock_in_store = {totallink.stock.getCoffeeId(c) for c in self.plus_coffees if
                                                     ...
                                                     self.plus_stores is None or len(
                                                         self.plus_stores) < 2 or self.plus_stores_combo.currentIndex() != 0 or len(
                                                         totallink.stock.getCoffeeCoffeeStocks(c)) == len(
                                                         self.plus_stores)}

            total_weight: float
            res, total_weight = totallink.blend.openCustomBlendDialog(self, self.aw, inWeight, self.aw.qmc.weight[2],
            ...
            if res is not None:  # not canceled
                ...
                if self.aw.qmc.plus_custom_blend.name.strip() == '' and self.plus_blend_selected_spec is not None and 'hr_id' in self.plus_blend_selected_spec and \
                        self.plus_blend_selected_spec['hr_id'] == '':
    ...
    def SetupSetDefaults(self, _: bool = False) -> None:
        ...
        if self.setup_ui is not None:
            ...
            self.aw.qmc.last_batchsize = convertWeight(self.aw.qmc.roastersize_setup, 1,
                                                       0)  # set last_batchsize to nominal batch size (kg) in g
            if self.weightinedit.text() == '0':
                nominal_batch_size = convertWeight(self.aw.qmc.roastersize_setup, 1,
    ...
    def defineBatchEditor(self) -> None:
        ...
        self.batchcounterSpinBox.setAlignment(
        ...
        self.batchposSpinBox.setAlignment(
            Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignTrailing | Qt.AlignmentFlag.AlignVCenter)
    ...
    def updatePlusSelectedLine(self) -> None:
        try:
            ...
            if self.plus_coffee_selected is not None and self.plus_coffee_selected_label:
                ...
                for i, ll in sorted(
                        zip(self.plus_blend_selected_spec['ingredients'], self.plus_blend_selected_spec_labels,
                            strict=True), key=lambda tup: tup[0]['ratio'], reverse=True)[:3]:
                    if first_component:
                        first_component = False
                    else:
                        line = line + ', '
                    # c = f"<a href=\"{totallink.util.coffeeLink(i['coffee'])}\"{dark_mode_link_color}>{abbrevString(ll, 18)}</a>"
                    # line = line + str(int(round(i['ratio'] * 100))) + '% ' + c
            # if line and len(
            #         line) > 0 and self.plus_store_selected is not None and self.plus_store_selected_label is not None:
            #     line = line + f'  (<a href="{totallink.util.storeLink(self.plus_store_selected)}"{dark_mode_link_color}>{self.plus_store_selected_label}</a>)'
        ...
        if self.aw.plus_account is not None:
            try:  # this can crash if dialog got closed while this is processed in a different thread!
                ...
                if storeIndex is None or storeIndex == -1:
                    self.plus_stores = totallink.stock.getStores()
                    try:
                        if len(self.plus_stores) == 1:
                            self.plus_default_store = totallink.stock.getStoreId(self.plus_stores[0])
                    ...
                    store_items = totallink.stock.getStoreLabels(self.plus_stores)
                    self.plus_stores_combo.addItems([''] + store_items)
                    p = (totallink.stock.getStorePosition(self.plus_default_store,
                                                          self.plus_stores) if self.plus_default_store is not None else None)
                    if p is None:
                ...
                if storeIdx > 0 and self.plus_stores is not None and len(self.plus_stores) > storeIdx - 1:
                    ...
                    selected_store_id = totallink.stock.getStoreId(selected_store)
                    if (
                            self.plus_coffee_selected or self.plus_blend_selected_spec) and self.plus_store_selected != selected_store_id:
                ...
                self.plus_coffees = totallink.stock.getCoffees(self.unitsComboBox.currentIndex(),
                                                               self.plus_default_store)
                ...
                custom_blend: totallink.stock.Blend | None = None
                ...
                if self.aw.qmc.plus_custom_blend is not None and self.aw.qmc.plus_custom_blend.name.strip() != '':
                    ...
                    if len(coffees) > 2 and self.aw.qmc.plus_custom_blend.isValid(coffees.values()):
                        custom_blend = totallink.stock.Blend(
                            hr_id='',
                            label=self.aw.qmc.plus_custom_blend.name.strip(),
                            ingredients=[totallink.stock.BlendIngredient(ratio=c.ratio, coffee=c.coffee) for c in
                                         self.aw.qmc.plus_custom_blend.components])
                self.plus_blends = totallink.stock.getBlends(self.unitsComboBox.currentIndex(), self.plus_default_store,
                                                             custom_blend)
                self.plus_blends_combo.blockSignals(True)
                self.plus_blends_combo.clear()
                self.plus_blends_combo.resetInverted()
                blend_items = totallink.stock.getBlendLabels(self.plus_blends)
                ...
                if self.plus_blend_selected_spec is not None and self.plus_store_selected is not None:
                    p = totallink.stock.getBlendSpecStockPosition(self.plus_blend_selected_spec,
        ...
        if b:
            if self.aw.app.darkmode:
                self.beansedit.setStyleSheet(
                    'QTextEdit { background-color: #0D658F; selection-background-color: darkgray; }')
            else:
                self.beansedit.setStyleSheet(
                    'QTextEdit { background-color: #e4f3f8; selection-background-color: darkgray;  }')
        else:
    ...
    def updateBlendLines(self, blend: totallink.stock.BlendStructure) -> None:
        if self.weightinedit.text() != '':
            weightIn = float(comma2dot(self.weightinedit.text()))
        else:
            weightIn = 0.0
        weight_unit_idx = self.unitsComboBox.currentIndex()
        blend_lines = totallink.stock.blend2beans(blend, weight_unit_idx, weightIn)
        self.beansedit.clear()
        for ll in blend_lines:
            self.beansedit.append(ll)

    def fillBlendData(self, blend: totallink.stock.BlendStructure, prev_coffee_label: str | None,
                      prev_blend_label: str | None) -> None:
        try:
    ...
    def fillCoffeeData(self, coffee: tuple[str, tuple[totallink.stock.Coffee, totallink.stock.StockItem]],
                       prev_coffee_label: str | None, prev_blend_label: str | None) -> None:
        try:
            cd = totallink.stock.getCoffeeCoffeeDict(coffee)
            self.beansedit.setPlainText(totallink.stock.coffee2beans(cd))
            keep_modified_moisture = self.modified_moisture_greens_text
            keep_modified_density = self.modified_density_in_text
            moisture_txt = '0'
            try:
    ...
    def getBlendDictCurrentWeight(self, blend: tuple[str, tuple[
        totallink.stock.Blend, totallink.stock.StockItem, float, dict[str, str], float, list[
            tuple[float, totallink.stock.Blend]]]]) -> totallink.stock.Blend:
        if self.weightinedit.text() != '':
            weightIn = float(comma2dot(self.weightinedit.text()))
        else:
            weightIn = 0.0
        ...
        return totallink.stock.getBlendBlendDict(blend, v)
    ...
    def blendSelectionChanged(self, n: int) -> None:
        ...
        if n < 1 or self.plus_blends is None:
            ...
            self.plus_blend_selected_spec = cast(totallink.stock.Blend, dict(bd))  # make a copy of the blend dict
            ...
            self.plus_blend_selected_spec_labels = [i.get('label', '') for i in
            ...
            for i in self.plus_blend_selected_spec['ingredients']:
                entry = totallink.stock.BlendIngredient(ratio=i['ratio'], coffee=i['coffee'])
                if 'ratio_num' in i:
            ...
            self.plus_amount_selected = totallink.stock.getBlendMaxAmount(selected_blend)
            self.plus_amount_replace_selected = totallink.stock.getBlendReplaceMaxAmount(selected_blend)
        ...
        if 0 < n <= len(self.aw.recentRoasts):
            ...
            if 'colorSystem' in rr:
                if rr['colorSystem'] in self.aw.qmc.color_systems:
                    ...
                    self.colorSystemComboBox.setCurrentIndex(self.aw.qmc.color_system_idx)
                elif isinstance(rr['colorSystem'], int) and rr['colorSystem'] < len(
                        self.aw.qmc.color_systems):  # type: ignore[unreachable] # compatibility to older versions where colorSystems could be an int
                    # to stay compatible with older versions were rr['colorSystem'] was an index instead of the name of a system
                    self.aw.qmc.color_system_idx = rr[
                        'colorSystem']  # type: ignore[unreachable] # compatibility to older versions where colorSystems could be an int
                    self.colorSystemComboBox.setCurrentIndex(self.aw.qmc.color_system_idx)
                ...
                if self.plus_blend_selected_spec is not None and self.plus_blends is not None and 'hr_id' in self.plus_blend_selected_spec:
                    ...
                    try:
                        pos_in_blends = next(i for i, b in enumerate(self.plus_blends) if \
                                             totallink.stock.getBlendId(b) == self.plus_blend_selected_spec['hr_id'] and
                                             totallink.stock.getBlendStockDict(b)[
        ...
        try:
            ...
            if title != QApplication.translate('Scope Title', 'Roaster Scope') and weightIn != 0:
                ...
                rr = self.aw.createRecentRoast(
                    ...
                    self.aw.qmc.roastbatchnr,
                    # self.batchcounterSpinBox # self.aw.superusermode and self.aw.qmc.batchcounter > -1
    ...
    def updateTarePopup(self, adjust_index: bool = True) -> None:
        ...
        with QSignalBlocker(
                self.tareComboBox):  # blocking all signals, especially its currentIndexChanged connected to tareChanged which would lead to cycles
    ...
    def initEnergyTab(self) -> None:
        ...
        if not self.tabInitialized[4]:
            ...
            self.energy_ui.copyTableButton.setToolTip(
                QApplication.translate('Tooltip', 'Copy table to clipboard, OPTION or ALT click for tabular text'))
            ...
            self.setButtonTranslations(self.energy_ui.loadsDefaultsButtons, 'Restore Defaults',
                                       QApplication.translate('Button', 'Restore Defaults'))
            self.energy_ui.loadlabelsLabel.setText(QApplication.translate('Label', 'Label'))
            self.energy_ui.loadlabelsLabel.setToolTip(QApplication.translate('Tooltip',
                                                                             "Load label for reference. If left blank the row label, 'A', 'B', 'C' or 'D' will be used in its place."))
            ...
            self.energy_ui.ratingunitsLabel.setToolTip(
                QApplication.translate('Tooltip', 'Unit of the load matching the power rating'))
            ...
            self.energy_ui.eventsLabel.setToolTip(QApplication.translate('Tooltip',
                                                                         'Data source for the load. Either a recorded event type or the internal PID duty signal.'))
            self.energy_ui.pressureLabel.setText(QApplication.translate('Label', 'Pressure %'))
            self.energy_ui.pressureLabel.setToolTip(QApplication.translate('Tooltip',
                                                                           'Ticked if readings are taken from a pressure gauge. Unticked for electric roasters and regular burner power output.'))
            self.energy_ui.electricEnergyMixLabel.setText(QApplication.translate('Label', 'Electric Energy Mix:'))
            self.energy_ui.electricEnergyMixLabel.setToolTip(QApplication.translate('Tooltip',
                                                                                    'Electric Energy Mix approximates the percentage of electricity that comes from renewable sources'))
            self.energy_ui.gasMixLabel.setText(QApplication.translate('Label', 'Gas Energy Mix:'))
            self.energy_ui.gasMixLabel.setToolTip(QApplication.translate('Tooltip',
                                                                         'Gas Energy Mix approximates the percentage of gas that comes from renewable sources'))
            ...
            self.setButtonTranslations(self.energy_ui.protocolDefaultsButton, 'Restore Defaults',
                                       QApplication.translate('Button', 'Restore Defaults'))
            ...
            self.energy_ui.BBPafterPreHeatcheckBox.setText(
                QApplication.translate('Label', 'Between Batches after Pre-Heating'))
            ...
            try:
                ...
                for combo in [self.energy_ui.events0, self.energy_ui.events1, self.energy_ui.events2,
                              self.energy_ui.events3]:
            ...
            # input validators
            regextime = QRegularExpression(
                r'^$|^[0-9]?[0-9]?[0-9]:[0-5][0-9]$')  # includes the empty string to trigger editingFinished; does not support hh:mm input!!
        ...
        self.energy_ui.roastbatchposLabel.setText(
            f"{QApplication.translate('Label', 'Batch')} #{self.aw.qmc.roastbatchpos}")
    ...
    def createEnergyDataTable(self) -> None:
        ...
        for i in range(ndata):
            if self.btu_list[i]['Kind'] in {0, 2, 4, 8, 10, 11, 12,
                                            13}:  # Preheat Measured, BBP Measured, Cooling Measured, Meter Batch/Preheat/BBP/Roast/Cooling
                ...
                load_widget = MyTableWidgetItemNumber(f"{self.btu_list[i]['load_pct']:.1f}% x\u0304",
                                                      self.btu_list[i]['load_pct'])
            else:
                load_widget = MyTableWidgetItemNumber(f"{self.btu_list[i]['load_pct']:.1f}%",
                                                      self.btu_list[i]['load_pct'])
            load_widget.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)

            if self.btu_list[i]['Kind'] in {0, 2, 4,
                                            10}:  # Preheat Measured, BBP Measured, Cooling Measured, Meter Preheat
                duration_mmss_widget = MyTableWidgetItemNumber('', 0)
            else:
                duration_mmss_widget = MyTableWidgetItemNumber(stringfromseconds(self.btu_list[i]['duration']),
                                                               self.btu_list[i]['duration'])
                duration_mmss_widget.setTextAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)

            BTUs = self.aw.qmc.convertHeat(self.btu_list[i]['BTUs'], 'BTU',
                                           self.aw.qmc.energyunits[self.aw.qmc.energyresultunit_setup])
            ...
            SourceType_widget = MyTableWidgetItemNumber(self.aw.qmc.sourcenames[self.btu_list[i]['SourceType']],
                                                        self.btu_list[i]['SortOrder'])
            SourceType_widget.setTextAlignment(Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignVCenter)

            Kind_widget = MyTableWidgetItemNumber(self.aw.qmc.kind_list[self.btu_list[i]['Kind']],
                                                  self.btu_list[i]['SortOrder'])
    ...
    def updateLoadRatings(self, updateMetrics: bool = True) -> None:
        self.aw.qmc.loadratings[0] = toFloat(scaleFloat2String(self.energy_ui.loadrating0.text())) if len(
            self.energy_ui.loadrating0.text()) > 0 else 0
        self.aw.qmc.loadratings[1] = toFloat(scaleFloat2String(self.energy_ui.loadrating1.text())) if len(
            self.energy_ui.loadrating1.text()) > 0 else 0
        self.aw.qmc.loadratings[2] = toFloat(scaleFloat2String(self.energy_ui.loadrating2.text())) if len(
            self.energy_ui.loadrating2.text()) > 0 else 0
        self.aw.qmc.loadratings[3] = toFloat(scaleFloat2String(self.energy_ui.loadrating3.text())) if len(
            self.energy_ui.loadrating3.text()) > 0 else 0
        if updateMetrics:
            self.updateMetricsLabel()
    ...
    def updateMeterSources(self, updateMetrics: bool = True) -> None:
        self.aw.qmc.metersources[0] = max(0,
                                          self.energy_ui.meter1SourceComboBox.currentIndex())  # could be -1 if never set
    ...
    def updateEnergyConfig(self) -> None:
        if self.tabInitialized[4]:
            self.aw.qmc.energyresultunit_setup = max(0,
                                                     self.energy_ui.resultunitComboBox.currentIndex())  # could be -1 if never set
            ## Details tab
            ## Loads tab
            # label
    ...
    def updateMetricsLabel(self) -> None:
        try:
            ...
            metrics, self.btu_list = self.aw.qmc.calcEnergyuse(
                self.weightinedit.text())  # pylint: disable=attribute-defined-outside-init
            if ('BTU_batch' in metrics and
                ...
                #
                total_energy = scaleFloat2String(self.aw.qmc.convertHeat(metrics['BTU_batch'], 'BTU',
                                                                         self.aw.qmc.energyunits[
                                                                             self.aw.qmc.energyresultunit_setup]))
                self.energy_ui.totalEnergyLabel.setText(f'{total_energy} {energy_unit}')
                #
                preheat_energy = scaleFloat2String(self.aw.qmc.convertHeat(metrics['BTU_preheat'], 'BTU',
                                                                           self.aw.qmc.energyunits[
                                                                               self.aw.qmc.energyresultunit_setup]))
                self.energy_ui.preheatEnergyLabel.setText(
                    f"{preheat_energy} {energy_unit} ({QApplication.translate('Label', 'Preheat')})")
                BBP_energy = scaleFloat2String(self.aw.qmc.convertHeat(metrics['BTU_bbp'], 'BTU',
                                                                       self.aw.qmc.energyunits[
                                                                           self.aw.qmc.energyresultunit_setup]))
                self.energy_ui.BBPEnergyLabel.setText(
                    f"{BBP_energy} {energy_unit} ({QApplication.translate('Label', 'BBP')})")
                roast_energy = scaleFloat2String(self.aw.qmc.convertHeat(metrics['BTU_roast'], 'BTU',
                                                                         self.aw.qmc.energyunits[
                                                                             self.aw.qmc.energyresultunit_setup]))
                self.energy_ui.roastEnergyLabel.setText(
                    f"{roast_energy} {energy_unit} ({QApplication.translate('Label', 'Roast')})")
                ...
                if KWH_per_green > 0:
                    ...
                    self.energy_ui.EnergyPerKgCoffeeLabel.setText(
                        f"{scaled_energy_kwh} {QApplication.translate('Label', 'per kg green coffee')}{mode}")
                ...
                if metrics['CO2_batch'] >= 0:
                    scaled_co2_batch = f"{scaleFloat2String(metrics['CO2_batch'])}g" if metrics[
                                                                                            'CO2_batch'] < 1000 else f"{scaleFloat2String(metrics['CO2_batch'] / 1000.)}kg"
                    self.energy_ui.totalCO2Label.setText(f'{scaled_co2_batch}')
                    #
                    scaled_co2_preheat = f"{scaleFloat2String(metrics['CO2_preheat'])}g" if metrics[
                                                                                                'CO2_preheat'] < 1000 else f"{scaleFloat2String(metrics['CO2_preheat'] / 1000.)}kg"
                    self.energy_ui.preheatCO2label.setText(
                        f"{scaled_co2_preheat} ({QApplication.translate('Label', 'Preheat')})")
                    scaled_co2_bbp = f"{scaleFloat2String(metrics['CO2_bbp'])}g" if metrics[
                                                                                        'CO2_bbp'] < 1000 else f"{scaleFloat2String(metrics['CO2_bbp'] / 1000.)}kg"
                    self.energy_ui.BBPCO2label.setText(f"{scaled_co2_bbp} ({QApplication.translate('Label', 'BBP')})")
                    scaled_co2_roast = f"{scaleFloat2String(metrics['CO2_roast'])}g" if metrics[
                                                                                            'CO2_roast'] < 1000 else f"{scaleFloat2String(metrics['CO2_roast'] / 1000.)}kg"
                    self.energy_ui.roastCO2label.setText(
                        f"{scaled_co2_roast} ({QApplication.translate('Label', 'Roast')})")
                    ...
                    if CO2_per_green > 0:
                        ...
                        self.energy_ui.CO2perKgCoffeeLabel.setText(
                            f"{scaled_co2_kg} {QApplication.translate('Label', 'per kg green coffee')}{mode}")
                    # no weight is available
                    else:
                        self.energy_ui.CO2perKgCoffeeLabel.setText('')
            ...
            _, _, exc_tb = sys.exc_info()
            self.aw.qmc.adderror(
                (QApplication.translate('Error Message', 'Exception:') + ' updateMetricsLabel() {0}').format(str(e)),
                getattr(exc_tb, 'tb_lineno', '?'))
    ...
    def updateEnergyUnitLabels(self) -> None:
        self.energy_ui.loadAUnitLabel.setText(self.formatLoadUnitLabel(
            max(0, self.energy_ui.ratingunit0.currentIndex())))  # curentIndex() returns -1 if never set
        self.energy_ui.loadBUnitLabel.setText(
            self.formatLoadUnitLabel(max(0, self.energy_ui.ratingunit1.currentIndex())))
        self.energy_ui.loadCUnitLabel.setText(
            self.formatLoadUnitLabel(max(0, self.energy_ui.ratingunit2.currentIndex())))
        self.energy_ui.loadDUnitLabel.setText(
            self.formatLoadUnitLabel(max(0, self.energy_ui.ratingunit3.currentIndex())))

    def updateEnableZHpct(self) -> None:
        for ew, zw, hw, pcb in [
            (self.energy_ui.events0, self.energy_ui.zeropcts0, self.energy_ui.hundredpct0,
             self.energy_ui.pressureCheckBox0),
            (self.energy_ui.events1, self.energy_ui.zeropcts1, self.energy_ui.hundredpct1,
             self.energy_ui.pressureCheckBox1),
            (self.energy_ui.events2, self.energy_ui.zeropcts2, self.energy_ui.hundredpct2,
             self.energy_ui.pressureCheckBox2),
            (self.energy_ui.events3, self.energy_ui.zeropcts3, self.energy_ui.hundredpct3,
             self.energy_ui.pressureCheckBox3),
        ]:
            idx = ew.currentIndex()
    ...
    def ratingunits_currentindexchanged(self) -> None:
        ...
        if isinstance(sender, QComboBox):
            try:
                i = [self.energy_ui.ratingunit0, self.energy_ui.ratingunit1, self.energy_ui.ratingunit2,
                     self.energy_ui.ratingunit3].index(sender)
    ...
    def sourcetypes_currentindexchanged(self) -> None:
        ...
        if isinstance(sender, QComboBox):
            try:
                i = [self.energy_ui.sourcetype0, self.energy_ui.sourcetype1, self.energy_ui.sourcetype2,
                     self.energy_ui.sourcetype3].index(sender)
    ...
    def load_etypes_currentindexchanged(self) -> None:
        ...
        if isinstance(sender, QComboBox):
            try:
                i = [self.energy_ui.events0, self.energy_ui.events1, self.energy_ui.events2,
                     self.energy_ui.events3].index(sender)
                self.aw.qmc.load_etypes[i] = max(0, sender.currentIndex())
                zw = [self.energy_ui.zeropcts0, self.energy_ui.zeropcts1, self.energy_ui.zeropcts2,
                      self.energy_ui.zeropcts3][i]
                zw.setEnabled(sender.currentIndex() in {1, 2, 3, 4})
                hw = [self.energy_ui.hundredpct0, self.energy_ui.hundredpct1, self.energy_ui.hundredpct2,
                      self.energy_ui.hundredpct3][i]
                hw.setEnabled(sender.currentIndex() in {-1, 0, 1, 2, 3, 4})
                pcb = \
                    [self.energy_ui.pressureCheckBox0, self.energy_ui.pressureCheckBox1,
                     self.energy_ui.pressureCheckBox2,
                     self.energy_ui.pressureCheckBox3][i]
    ...
    def pressureCheckBox_statechanged(self, _: int) -> None:
        ...
        if isinstance(sender, QCheckBox):
            try:
                i = [self.energy_ui.pressureCheckBox0, self.energy_ui.pressureCheckBox1,
                     self.energy_ui.pressureCheckBox2, self.energy_ui.pressureCheckBox3].index(sender)
    ...
    def energyTabSwitched(self, i: int) -> None:
        ...
        if i == 0:
            ...
            # save column widths
            self.aw.qmc.energytablecolumnwidths = [self.energy_ui.datatable.columnWidth(c) for c in
                                                   range(self.energy_ui.datatable.columnCount())]
    ...
    def validatePctText(self, s: str) -> str:
        ...
        try:
            if s == s.strip('%'):
                ...
                if f < 0:
                    res = str(abs(int(
                        f * 1000. / 10))) + '%'  # using 1000/10 to get around Pythons decimal error ex. .58*100 = 57.999
                else:
    ...
    def populateSetupDefaults(self) -> None:
        if self.setup_ui is not None:
            ...
            self.setup_ui.labelMachineSizeDefault.setText(
                f'{self.aw.qmc.roastertype_setup} {render_weight(self.aw.qmc.roastersize_setup, 1, weight_units.index(self.aw.qmc.weight[2]))}')
    ...
    def initSetupTab(self) -> None:
        if not self.tabInitialized[5]:
            ...
            self.setup_ui.doubleSpinBoxRoasterSize.setToolTip(
                QApplication.translate('Tooltip', 'The maximum nominal batch size of the machine in kg'))
            ...
            self.setButtonTranslations(self.setup_ui.Defaults, 'Restore Defaults',
                                       QApplication.translate('Button', 'Restore Defaults'))
    ...
    def inWeight(self, _: bool, overwrite: bool = False) -> None:
        QTimer.singleShot(1, lambda: self.setWeight(self.weightinedit, self.bean_density_in_edit,
                                                    self.moisture_greens_edit, overwrite))

    @pyqtSlot(bool)
    def outWeight(self, _: bool = False, overwrite: bool = False) -> None:
        QTimer.singleShot(1, lambda: self.setWeight(self.weightoutedit, self.bean_density_out_edit,
                                                    self.moisture_roasted_edit, overwrite))
    ...
    def setWeight(self, weight_edit: QLineEdit, density_edit: QLineEdit | None, moisture_edit: QLineEdit | None,
        ...
        if w is not None and w > -1:
            ...
            if overwrite:
                ...
                self.scale_set = convertWeight(new_w, weight_units.index(self.aw.qmc.weight[2]),
                                               0)  # convert to weight units
    ...
    def labelOriginFlagChanged(self, _: int = 0) -> None:
        totallink.stock.coffee_label_normal_order = self.label_origin_flag.isChecked()
        totallink.stock.clearStockCaches()  # we need to clear the stock caches to receive the updated coffee labels
    ...
    def createDataTable(self) -> None:
        ...
        self.datatable.setSelectionMode(
            QTableWidget.SelectionMode.ExtendedSelection)  # QTableWidget.SelectionMode.SingleSelection, ContiguousSelection, MultiSelection
        ...
        for i in range(ndata):
            ...
            deltaET_str = '--'
            try:
                if i > 0 and (self.aw.qmc.timex[i] - self.aw.qmc.timex[i - 1]) and self.aw.qmc.temp1[i] != -1 and \
                        self.aw.qmc.temp1[i - 1] != -1:
                    rateofchange1 = 60 * (self.aw.qmc.temp1[i] - self.aw.qmc.temp1[i - 1]) / (
                            self.aw.qmc.timex[i] - self.aw.qmc.timex[i - 1])
                    if len(self.aw.qmc.DeltaETfunction):
                        try:
                            rateofchange1 = self.aw.qmc.eval_math_expression(self.aw.qmc.DeltaETfunction,
                                                                             self.aw.qmc.timex[i], RTsname='R1',
                                                                             RTsval=rateofchange1)
                        except Exception:  # pylint: disable=broad-except
                            pass
            ...
            deltaBT_str = '--'
            try:
                if i > 0 and (self.aw.qmc.timex[i] - self.aw.qmc.timex[i - 1]) and self.aw.qmc.temp2[i] != -1 and \
                        self.aw.qmc.temp2[i - 1] != -1:
                    rateofchange2 = 60 * (self.aw.qmc.temp2[i] - self.aw.qmc.temp2[i - 1]) / (
                            self.aw.qmc.timex[i] - self.aw.qmc.timex[i - 1])
                    if len(self.aw.qmc.DeltaBTfunction):
                        try:
                            rateofchange2 = self.aw.qmc.eval_math_expression(self.aw.qmc.DeltaBTfunction,
                                                                             self.aw.qmc.timex[i], RTsname='R2',
                                                                             RTsval=rateofchange2)
                        except Exception:  # pylint: disable=broad-except
                            pass
            ...
            try:
                ...
                for i in range(nevents):
                    ...
                    if self.aw.qmc.timeindex[0] > -1 and len(self.aw.qmc.timex) > self.aw.qmc.timeindex[0]:
                        timez = stringfromseconds(self.aw.qmc.timex[self.aw.qmc.specialevents[i]] - self.aw.qmc.timex[
                            self.aw.qmc.timeindex[0]])
                    else:
            ...
            try:
                ...
                for i in range(nevents):
                    try:
                        ...
                        time_idx: int
                        if self.aw.qmc.timeindex[0] > -1:
                            time_idx = self.aw.qmc.time2index(
                                self.aw.qmc.timex[self.aw.qmc.timeindex[0]] + stringtoseconds(str(timez.text())))
                        else:
    ...
    def createAlarmEventRows(self, rows: list[int]) -> None:
        for r in rows:
            ...
            if TP:
                ...
                self.aw.qmc.alarmtemperature.append(
                    float(round(self.aw.qmc.temp2[self.aw.qmc.specialevents[r]])))  # the BT trigger temperature
    ...
    def createAlarmEventTable(self, _: bool = False) -> None:
        if len(self.aw.qmc.specialevents):
            ...
            if selected and len(selected) > 0:
                ...
                message = QApplication.translate('Message', 'Alarms from events #{0} created').format(
    ...
    def deleteEventTable(self, _: bool = False) -> None:
        if len(self.aw.qmc.specialevents):
            ...
            if selected and len(selected) > 0:
                ...
                message = QApplication.translate('Message', ' Event #{0} deleted').format(
                    str(len(self.aw.qmc.specialevents) + 1))
            self.aw.qmc.fileDirty()
            self.createEventTable(force=True)
            self.aw.qmc.redrawKeepViewSignal.emit(
                False,  # recomputeAllDeltas (default: True)
    ...
    def weightouteditChanged(self) -> None:
        ...
        self.percent()
        self.defect_percent()
        if ((self.bean_density_out_edit.text() in {'0', ''} and self.volumeoutedit.text() not in {'0',
                                                                                                  ''} and self.weightoutedit_text().strip() not in {
                 '0', ''}) or
        ...
        if self.plus_amount_replace_selected is not None:
            try:
                ...
                if weightUnit == 'kg':
                    ...
                    wc = convertWeight(weightIn, weight_units.index(self.unitsComboBox.currentText()),
    ...
    def weightineditChanged(self) -> None:
        ...
        if ((self.bean_density_in_edit.text() in {'0', ''} and self.volumeinedit.text() not in {'0',
                                                                                                ''} and self.weightinedit.text().strip() not in {
                 '0', ''}) or
    ...
    def density_percent(self) -> None:
        ...
        try:
            if self.bean_density_out_edit.text() != '' and float(comma2dot(self.bean_density_out_edit.text())) != 0.0:
                percent = self.aw.weight_loss(float(comma2dot(str(self.bean_density_in_edit.text()))),
                                              float(comma2dot(str(self.bean_density_out_edit.text()))))
        except Exception:  # pylint: disable=broad-except
            pass
        if percent <= 0:
    ...
    def percent(self) -> None:
        ...
        try:
            if self.weightoutedit_text() != '' and float(comma2dot(self.weightoutedit_text())) != 0.0:
                percent = self.aw.weight_loss(float(comma2dot(self.weightinedit.text())),
                                              float(comma2dot(self.weightoutedit_text())))
        except Exception:  # pylint: disable=broad-except
            pass
        if percent > 0:
            percentstring = f'-{float2float(percent, self.aw.percent_decimals)}%'
    ...
    def defect_percent(self) -> None:
        ...
        try:
            defects: float = (
                float(comma2dot(self.weightoutdefectsedit.text())) if self.weightoutdefectsedit.text() != '' else 0)
    ...
    def volume_percent(self) -> None:
        ...
        try:
            if self.volumeoutedit.text() != '' and float(comma2dot(self.volumeoutedit.text())) != 0.0:
                percent = self.aw.volume_increase(float(comma2dot(self.volumeinedit.text())),
                                                  float(comma2dot(self.volumeoutedit.text())))
        except Exception:  # pylint: disable=broad-except
            pass
        if percent == 0:
            ...
            if density_in != 0:
                if self.weightinedit.text() != '':  # and self.volumeinedit.text().strip() in {'0',''}: # prefer to recompute volume which is seldom measured
                    ...
                    if weight_in != 0:
                        ...
                        volume_in = convertVolume(volume_in, volume_units.index('l'),
                                                  self.volumeUnitsComboBox.currentIndex())
                ...
                if not keep_weight_in and self.volumeinedit.text() != '' and self.weightinedit.text().strip() in {'0',
                    ...
                    if volume_in != 0:
                        ...
                        volume_in = convertVolume(volume_in, self.volumeUnitsComboBox.currentIndex(),
                                                  volume_units.index('l'))
        ...
        if self.bean_density_out_edit.text() != '':
            ...
            if density_out != 0:
                if self.weightoutedit_text() != '':  # and self.volumeoutedit.text().strip() in {'0',''}:
                    ...
                    if weight_out != 0:
                        weight_out = convertWeight(weight_out, self.unitsComboBox.currentIndex(),
                                                   weight_units.index('g'))
                        volume_out = weight_out / density_out  # in g/l
                        # convert to selected volume unit
                        volume_out = convertVolume(volume_out, volume_units.index('l'),
                                                   self.volumeUnitsComboBox.currentIndex())
                ...
                if self.volumeoutedit.text() != '' and self.weightoutedit.text().strip() in {'0', ''}:
                    ...
                    if volume_out != 0:
                        ...
                        volume_out = convertVolume(volume_out, self.volumeUnitsComboBox.currentIndex(),
                                                   volume_units.index('l'))
                        weight_out = volume_out * density_out  # in g/l
                        weight_out = convertWeight(weight_out, weight_units.index('g'),
        ...
        if self.chargeedit.text() == '':
            ...
            try:
                if stringtoseconds(str(self.chargeedit.text())) > 0 and self.aw.qmc.timeindex[0] != -1:
                    startindex = self.aw.qmc.time2index(
                        self.aw.qmc.timex[self.aw.qmc.timeindex[0]] + stringtoseconds(str(self.chargeedit.text())))
                    timeindex_before = self.aw.qmc.timeindex[0]
                    self.aw.qmc.timeindex[0] = max(-1, startindex)
                    self.aw.qmc.startofx += (
                            self.aw.qmc.timex[self.aw.qmc.timeindex[0]] - self.aw.qmc.timex[timeindex_before])
                # if there is a CHARGE recorded and the time entered is negative. Use relative time
                    ...
                    relativetime = self.aw.qmc.timex[self.aw.qmc.timeindex[0]] - abs(
                        stringtoseconds(str(self.chargeedit.text())))
                ...
                # if there is _no_ CHARGE recorded and the time entered is negative. ERROR
                    ...
                    self.aw.qmc.adderror(
                        QApplication.translate('Error Message', 'Unable to move CHARGE to a value that does not exist'))
    ...
    def close_OK(self) -> None:
        ...
        if w0 == 0 and self.aw.qmc.last_batchsize == 0:
            ...
            self.aw.qmc.last_batchsize = convertWeight(w0, weight_units.index(w2),
                                                       0)  # remember last used batch size (in g)
        try:
            ...
            if self.aw.qmc.flagstart and not self.aw.qmc.title_show_always:
                ...
                if self.aw.qmc.background and self.aw.qmc.title != '':
                    if self.aw.qmc.roastbatchnrB == 0:
                        ...
                        titleB = self.aw.qmc.roastbatchprefixB + str(
                            self.aw.qmc.roastbatchnrB) + ' ' + self.aw.qmc.titleB
        ...
        if not self.aw.qmc.flagon:
            self.aw.sendmessage(
                QApplication.translate('Message', 'Roast properties updated but profile not saved to disk'))
        ...
        if (self.aw.qmc.flagstart and self.aw.qmc.safesaveflag and self.aw.qmc.timeindex[0] > -1 and
                self.aw.qmc.timeindex[6] > 0 and
                self.aw.plus_account is not None and not bool(self.aw.simulator)):
            try:
                totallink.queue.addRoast()
            except Exception as e:  # pylint: disable=broad-except
                _log.exception(e)
    ...
    def preHeatToolButton_triggered(self, _: bool = False) -> None:
        ...
        fields = [self.energy_ui.preheatenergies0,
                  ...
                  self.energy_ui.preheatenergies2,
                  self.energy_ui.preheatenergies3]
        self.getMeasuredvalues(title, self.updatePreheatEnergies, fields, loadEnergy, self.updatePreheatDuration,
                               self.energy_ui.preheatDuration, duration)
    ...
    def betweenBatchesToolButton_triggered(self, _: bool = False) -> None:
        ...
        self.getMeasuredvalues(title, self.updateBetweenBatchesEnergies, fields, loadEnergy,
                               self.updateBetweenBatchesDuration, self.energy_ui.betweenBatchesDuration, duration)
    ...
    def coolingToolButton_triggered(self, _: bool = False) -> None:
        ...
        fields = [self.energy_ui.coolingenergies0,
                  ...
                  self.energy_ui.coolingenergies2,
                  self.energy_ui.coolingenergies3]
        self.getMeasuredvalues(title, self.updateCoolingEnergies, fields, loadEnergy, self.updateCoolingDuration,
                               self.energy_ui.coolingDuration, duration)

    def openEnergyMeasuringDialog(self, title: str, loadLabels: list[str], loadValues: list[str], loadUnits: list[str],
                                  protocolDuration: str) -> int:
...
class CoffeesComboBox(StockComboBox):
    ...
    def getItems(self, unit: int) -> list[str]:
        plus_coffees = totallink.stock.getCoffees(unit, self.parentDialog.plus_default_store)
        return [''] + totallink.stock.getCoffeesLabels(plus_coffees)
        ...
        if self.parentDialog.aw.qmc.plus_custom_blend is not None and self.parentDialog.aw.qmc.plus_custom_blend.name.strip() != '':
            ...
            if len(coffees) > 2 and self.parentDialog.aw.qmc.plus_custom_blend.isValid(coffees.values()):
                custom_blend = totallink.stock.Blend(
                    ...
                    ingredients=[totallink.stock.BlendIngredient(ratio=c.ratio, coffee=c.coffee) for c in
                                 self.parentDialog.aw.qmc.plus_custom_blend.components])
        plus_blends = totallink.stock.getBlends(unit, self.parentDialog.plus_default_store, custom_blend)
        blend_items: list[str] = totallink.stock.getBlendLabels(plus_blends)
        return [''] + blend_items
